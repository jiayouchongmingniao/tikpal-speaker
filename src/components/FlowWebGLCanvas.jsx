...
void main() {
  ...
  float band = smoothstep(wake, 0.0, abs(centered.y - ridge));

  float glowWave = wave(centered.x, 0.36, 3.8, 0.22 + u_high * 0.08, 2.4);
  float aura = smoothstep(0.95, 0.0, length(vec2(centered.x * 0.85, centered.y - glowWave * 0.45)));
  ...
  float vignette = smoothstep(1.55, 0.2, length(centered * vec2(1.0, 0.82)));
